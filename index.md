...
<head>
    ...
    <style>
        /* Ajuste fino para evitar que o clique selecione texto no mobile e melhore o feedback de toque */
        .card-jogo {
...
<body class="bg-slate-100 text-slate-800 font-sans antialiased pb-28 relative min-h-screen">
    ...
    <header class="bg-blue-700 text-white shadow-lg sticky top-0 z-50 border-b-4 border-white">
        <div class="max-w-md mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <div class="relative inline-block text-left">
                <select id="select-lang" class="bg-white/10 text-white text-xs font-bold rounded-xl px-3 py-2 border border-white/20 focus:outline-none focus:ring-2 focus:ring-white cursor-pointer appearance-none pr-7 bg-[url('data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23ffffff%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%27style%3E')] bg-[length:8px_auto] bg-[right_10px_center] bg-no-repeat">
    ...
    <div class="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md p-4 border-t-2 border-slate-200 shadow-xl z-50">
        <div class="max-w-md mx-auto">
            <button id="btn-compartilhar" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-black uppercase tracking-wider py-4 px-4 rounded-2xl shadow-lg shadow-blue-600/20 transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2.5 text-sm">
                <span>💬</span>
    ...
    <script>
        const i18n = {
            ...
            en: {
                ...
                instruction: "Select the matches you plan to watch to build your schedule:",
                btnText: "Share Schedule",
                whatsappHeader: "👋 Check out the World Cup matches I'm planning to watch! Let's watch together?\n\n",
                whatsappFooter: "Which of these are you watching too?",
        ...
        function renderizarJogos() {
            ...
            jogos.forEach(jogo => {
                ...
                // Estilização inspirada nas cores azul e branco
                card.className = `card-jogo p-4 rounded-2xl border transition-all duration-200 cursor-pointer flex items-center justify-between select-none ${
                    isSelected 
                    ? 'bg-blue-600 text-white border-blue-700 shadow-md transform -translate-y-0.5' 
                ...
                card.innerHTML = `
                    <div class="flex-1">
                        <div class="text-[10px] font-black tracking-widest uppercase flex items-center gap-2 ${isSelected ? 'text-blue-200' : 'text-slate-400'}">
                            <span>📅 ${diaTraduzido}, ${jogo.data}</span>
                            <span>•</span>
                            <span>⏰ ${jogo.hora}</span>
                        </div>
                        <div class="text-base font-extrabold mt-1 flex items-center gap-2">
                            <span>${jogo.selecao1[currentLang]}</span>
                            <span class="${isSelected ? 'text-blue-300' : 'text-slate-300'} font-medium text-sm">×</span>
                    ...
                    <div class="ml-4 flex items-center justify-center">
                        <div class="w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all duration-200 ${
                            ...
                            ${isSelected ? '<svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="4" d="M5 13l4 4L19 7"></path></svg>' : ''}
                        </div>
                    </div>
                `;
        ...
        function atualizarBotao() {
            ...
            if (qtd === 0) {
                ...
                elBtnCompartilhar.classList.add('opacity-40', 'cursor-not-allowed');
                elBtnCompartilhar.classList.remove('shadow-blue-600/20', 'active:scale-[0.98]');
            } else {
                elBtnCompartilhar.disabled = false;
                elBtnCompartilhar.classList.remove('opacity-40', 'cursor-not-allowed');
                elBtnCompartilhar.classList.add('shadow-blue-600/20', 'active:scale-[0.98]');
            }
        }
